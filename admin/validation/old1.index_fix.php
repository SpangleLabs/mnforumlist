...
mysql_query("UPDATE ".$db_pre."data SET Forums = '$forums' WHERE NatID = '$i'");
$fixed++;
unset($forums);
...
mysql_query("UPDATE ".$db_pre."data SET Maps = '$maps' WHERE NatID = '$i'");
$fixed++;
unset($maps);
...
mysql_query("UPDATE ".$db_pre."data SET Logs = '$logs' WHERE NatID = '$i'");
$fixed++;
unset($logs);
...
if($file != '.' && $file != '..'){
$counter++;
$file_in_db = mysql_num_rows(mysql_query('SELECT * FROM '.$db_pre.'culture WHERE NatID = \''.$i.'\' AND File_name = \''.$file.'\''));
}}
closedir($folder);
$cultural_items = mysql_num_rows(mysql_query('SELECT * FROM '.$db_pre.'culture WHERE NatID = \''.$i.'\''));
if($counter!=$cultural_items) {$errors++;
echo("Error ".$errors.": Wrong number of cultural items listed in culture table for row ".$i.". (validator can't fix this atm. coder is too lazy...)\n"); }
...
mysql_query("UPDATE ".$db_pre."data SET Culture_items = '$culture_items' WHERE NatID = '$i'");
$fixed++;
unset($culture_items);
...
$PPD = round($PPD,3);
mysql_query("UPDATE ".$db_pre."data SET PPD = '$PPD' WHERE NatID = '$i'");
$fixed++;
}} else {
if($listed_PPD[PPD]!=-123456.789) {
$errors++;
echo("Error ".$errors.": PPD listed for row ".$i." in data table when activity logs were not made.\n");
mysql_query("UPDATE ".$db_pre."data SET PPD = '-123456.789' WHERE NatID = '$i'");
$fixed++;
}}
...
<a href=index_check.php>Click here to fix these problems</a>");
