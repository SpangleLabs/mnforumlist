...
$sql_tags = 'SELECT 
 tags_tagged.Tag_ID as Tag_ID,
 tags_data.Name as Name,
 tags_data.Description as Description 
 FROM `'.$db_pref.'tags_tagged` tags_tagged 
 LEFT JOIN `'.$db_pref.'tags_data` tags_data ON tags_data.ID = tags_tagged.Tag_ID 
 WHERE tags_tagged.NatID = \''.$_GET['ID'].'\' AND tags_tagged.Current = \'Y\'
 ORDER BY tags_tagged.Date_added ASC';
$raw_tags = mysql_query($sql_tags);
$num_tags = mysql_num_rows($raw_tags);
$list_tags = '';
for($a=0;$a<$num_tags;$a++) {
$dat_tags = mysql_fetch_array($raw_tags);
if($a!=0) {
$list_tags .= ', ';
}
$list_tags .= '<abbr title="'.$dat_tags['Description'].'"><a href="page_category.php?ID='.$dat_tags['Tag_ID'].'">'.$dat_tags['Name'].'</a></abbr>';
}
unset($raw_data,$num_data,$raw_tags,$num_tags,$dat_tags,$a);
...
<b>'.function_lang('DATA_CATEGORIES',$conf_lang,$db_pref).':</b> '.$list_tags.'<br />
